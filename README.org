...
- Important [0/9]
  ...
  - [ ] Polling
    - [ ] Self for state to continue a workflow
    - [ ] API to check for desired response to continue (maybe including fetching the results)
  - [ ] Callbacks
    - How does a node wait for a callback? Wait for state?
